...
打开 <http://localhost:5180>。收件箱里有一个待审批请求（批准它，或附评论拒绝）、一个规则拒绝、一个等待人工补全实现的操作和一个失败的任务（知悉它们，或送回上游，看修订轮次带着你的评论运行）。概览页显示最新快照及其观测盲区；“问题与任务”页列出每一轮；“追踪”打开某一轮背后的完整记录。
...
首次定时快照会接受审查并自动启动调查。你也可以在控制台上报一个问题，打开它的追踪，在收件箱批准被挂起的队列清空操作，然后点击**导出**，把整个会话保存为一个 JSON 文件。
...
Web 控制台的收件箱分四类——待反馈调查（补充信息并继续调查）、待审批请求（批准，或附评论拒绝）、已拒绝（谁拒绝的、为什么；送回上游或知悉）、失败（任务和操作；同样的两种决定）——此外还有带观测盲区的最新快照、带反馈和记录链接的问题与任务、实时事件流、冻结/恢复控制、运行期间显示实时进度的人工上报表单、正在运行的轮次及各自的“中断”按钮，以及模型中继迄今的花费与上限的对比。每个决定都会记录操作员的名字。
...
对于“已完成／仅诊断”但 Issue 仍等待人工的轮次，收件箱和“问题与任务”页面提供“发送反馈并继续调查”。评论会绑定当前轮次，并进入基于新快照的下一轮；旧页面重复发送会被拒绝。已有审批、拒绝或失败事项仍由对应入口处理。关闭说明、关闭人和时间会从事件记录中显示，旧会话中的说明也会保留。

CLI 对应命令为 `issues feedback <issue-id> --job <job-id> --comment "..." --as operator`；TUI 在待反馈项按 `b`。归档或已关闭的问题不能继续派发。

## 快照审查与自动入队

每次手动或定时采集都会先保存快照，再由 Snapshot Judge 审查一次。配置了模型后，审查 agent 复用运维团队的中继模型，但读取独立、脱敏且经过哈希绑定的 Judge View。健康状态、观测盲区和已有告警的规则检查始终运行，模型通过一个仅用于报告发现的工具补充关联分析。观测缺失会被标为可观测性问题，不会直接当作宕机。审查记录和 token 用量会保存，用量计入现有花费上限；模型失败时仍采用规则发现，审查失败不会丢弃已保存的快照。

Scheduler 校验发现，并按稳定的问题键合并到已有的未关闭 Issue。运行 `serve` 时，独立的入队处理循环会按优先级派发新 Issue，进入已有的 Operate、权限矩阵和验证流程。冻结期间只入队，队列跨重启保留，恢复后用新采集的证据开展调查。重复证据不会为正在处理的 Issue 再开一轮初始 Job；关闭前采集的旧证据晚到时不会重新打开问题，关闭后的新快照发现复发则可以创建新 Issue。

只有手动和定时快照触发审查；人工上报、反馈、探针请求、操作验证和派发前采集的快照不会递归触发审查。Web 概览会显示审查进度、警告、相关 Issue，以及输入和审查记录的链接；TUI 显示最近的审查状态。独立的 `snapshot` 命令负责采集、审查和入队，队列由 `serve` 处理。自动调查继续遵守原有运行手册权限与演练模式设置。
...
```bash
cargo run -- inbox                                            # 待反馈、审批、拒绝和失败事项
...
Job 的 `Completed` 表示本轮调查或观测完成，不代表 Issue 已解决。只读操作记录 `observation` 证据。整轮提议全部处理、相关操作均结束后，经过验证的修复或调度器确认的 `solved` 结论才允许自动结案；若本轮请求了后续验证轮，Issue 会保持未解决，直到后续轮得出结论。

全冻结期间仍可批准操作：批准会持久化，操作以 `Ready` 状态进入**执行队列**，在 Web 控制台、TUI 和 `inbox` 命令中可见。点击**恢复**后，系统执行并验证队列中的操作，无需再次批准。排队操作在控制器重启后仍保留；已在执行而结果未知的操作仍进入恢复审核。队列执行期间再次全冻结，会允许已开始的操作结束，并把其余操作留到下次恢复。

授权是对整个提议做出的裁定：运行手册、每个目标的类型（判题节点重启不能指向 API 服务端）、任务的目标范围和能力、参数——然后才是矩阵行。每一次状态转换都是比较并交换（compare-and-set），幂等键被原子地占用（与仍在进行的操作重复会被拒绝；失败后的重试被允许，但会升级为需要审批），超时的命令会连同整个进程组一起被杀掉，然后才报告失败。验证按操作类别进行且分等级：效果被观测到发生变化为 `strong`，目标本来就健康为 `weak`，什么都没执行为 `dry_run`，只读检查完成为 `observation`——只有真实修复证据才能解决一个问题；否则问题会等待人工关闭。
...
操作提议的 runbook 未注册，或已获权限的 runbook 没有配置命令时，会以 `WaitingForHuman` 进入 Inbox 的待反馈分组，尚未执行任何命令。人工补全经过审核的实现、提供替代方案或人工处理结果后，可送回上游重新评估；直接审批不能绕过实现缺口。实际执行失败和规则拒绝仍分别处理。
...
**Token 与费用。** 每次模型请求尝试都有稳定 ID 和持久化的 `model.request_started` / `model.request_finished` 事件；每次响应入账后才允许下一次请求。Job 最终用量是汇总快照，不会再次计费；旧版仅按轮记录的 `model.usage` 日志仍可读取。重试和发送后的取消会计入请求次数，拿不到用量则标为未知；本地拒绝、发送前取消不计 API 请求。响应内容解析失败也会保留能够读取的 usage。重启后未完成请求记为“被中断、用量未知”，导入归档不计入当前部署预算。

`[model.pricing]` 分别计算普通输入、缓存输入和输出费用。API 包含逐次明细；Web 根据账本事件实时刷新，TUI 展示逐次用量，CLI 在每次请求完成后打印更新。标有 ≥ 的费用只包括已知部分。`check-model` 连通性诊断也会将调用记入所选数据目录。

```bash
cargo run -- usage          # 总量、按模型、与上限对比
```

**预算。** `max_tokens_per_run` 到达已知 Token 上限后停止发起新请求，不增加收费的收尾调用；回合数和工具次数预算保留原有收尾机制。`[budget]` 在每次响应入账后检查，到限即冻结后续派发，并取消其他仍在进行的模型请求。已经收到的完整响应仍可提交结论。调高上限并手动恢复后才能继续。服务未返回或未报告的用量无法预知，因此预算机制不保证最终账单绝不超出上限。
...
- 模型驱动的调度器决策：harness、其 OpenAI 兼容客户端和 harness 驱动的运维团队都已实现并接到中继上，但模型驱动的调度器策略适配器尚未接入；候选入队、去重与派发由确定性规则校验。
...
模型错误、重试耗尽、空响应或上下文预检拒绝会产生失败运行报告，保留指令、完整对话、已完成回合及已知用量。transcript 包含失败时间、阶段、回合和可见的失败说明。适配器先归档，再提交最终 Job 结果；失败运行中尚未执行的提议不会下发。失败会话使用相同的 JSON 导出、导入及只读归档。如果产物存储本身失败，Job 会明确标出会话不完整，并在事件存储仍可用时记录已知开销。
