...
A completed Job means its investigation or observation finished; it does not by itself resolve the Issue. Read-only ActionRuns record `observation` evidence. Automatic resolution requires verified remediation or a Scheduler-confirmed `solved` result.
