...
Completed diagnosis-only investigations that still need human input appear in the Inbox and expose a feedback form in Issues & Jobs. Comments are bound to the displayed pass and carried into a new pass over a fresh Snapshot; stale submissions are rejected. Pending approval, denial, and failure decisions keep their existing controls. Closure comments, operator identity, and time are projected from the event log, including older sessions.

The CLI command is `issues feedback <issue-id> --job <job-id> --comment "..." --as operator`; the TUI uses `b` on a waiting-input item. Closed and archived Issues cannot be continued.
...
```bash
cargo run -- inbox                                            # human feedback, approvals, denials, and failures
...
### Failed model runs retain their history

A backend error, exhausted retries, an empty response or a context preflight refusal now ends in a failed run report that retains instructions, complete conversation entries, completed turns and known usage. Its transcript includes a failure timestamp, stage and turn, plus a visible failure notice. The adapter archives it before delivering the final Job result; unexecuted proposals from a failed run are discarded. Failed sessions use the same JSON export/import and read-only archive as successful sessions. If artifact storage itself fails, the Job explicitly says the archive is incomplete and still records known spending where the event store remains available.
